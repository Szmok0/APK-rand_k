...
type Props = {
  ...
  dim?: boolean;
  // Percentage-based sizing (e.g. { width: '70%', height: '70%' }) for a
  // caller that wants the icon to fill most of a parent box regardless of
  // that box's own pixel size, instead of a fixed `size` in px.
...
export function GlyphIcon({ glyphId, size = 28, dim, style }: Props) {
  const source = GLYPH_ICONS[glyphId];
  if (!source) return <View style={{ width: size, height: size }} />;
  return (
    <Image
      source={source}
      style={[{ width: size, height: size, opacity: dim ? 0.35 : 1 }, style]}
      resizeMode="contain"
    />
  );
}
